...
- **Mobile App (`artifacts/field-staff`):** An Expo mobile app for Android/iOS.
    ...
    - **Reporting:** Daily Outcome Reports for staff with WhatsApp sharing, Admin Ride Reports (CSV export), Staff List Excel export (GET /api/admin/staff/export — styled workbook with 16 columns, download button in both admin panel and mobile admin staff screen), **Daily KM Summary** (GPS vs Odometer comparison for all field staff on a given date — Section 4 in mobile admin Reports screen, with CSV + Excel download, variance color-coded per staff card), **Monthly Attendance Report** (Section 5 — per-staff: present/late/absent/leave days, avg check-in/out, total GPS KM, casual+sick leave balance; month navigator + Excel download; GET /api/admin/reports/monthly-attendance and /xlsx).
    - **Attendance Control (v1.8):** Admin mobile `/(admin)/attendance-control` screen (link from dashboard card). Two tabs: (1) Shift Settings — field + center staff shift start/end times (HH:MM), late grace period (0–120 min), late threshold preview, save to DB; (2) Corrections — list of manual attendance corrections with original→corrected times, reason, audit trail; FAB to add new correction (staff picker, date, corrected check-in/out HH:MM, reason). DB: `attendance_corrections` table. API: GET/PATCH /api/admin/attendance-settings, GET /api/admin/attendance/corrections, POST /api/admin/attendance/correct. Late mark rule stored in companies table (field_shift_start, field_shift_end, center_shift_start, center_shift_end, late_grace_minutes) and applied in Monthly Attendance Report.
    ...
    - **Document Capture (v5):** Simple `expo-image-picker` bottom sheet — "📷 Take Photo" or "🖼️ Choose from Gallery". No WebView, no native plugin, no multi-pass processing. Works identically on Expo Go and EAS builds. Replaced the previous WebView+OpenCV pipeline and `react-native-document-scanner-plugin` (both removed).
